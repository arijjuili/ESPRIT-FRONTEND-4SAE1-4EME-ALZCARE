...
## Session 29 (2026-03-04) - Community Enhancements: Popularity Score + Anti-Toxicity

### Feature: Popularity Score System
**Purpose:** Show trending posts based on engagement metrics and provide real-time content ranking.

**Backend Formula:** `score = (likeCount × 2) + (commentCount × 5) + max(0, 100 - hoursSincePost)`

**Features Implemented:**
- ✅ **Trending Tab** - New "Trending" tab alongside "Latest" for sorting posts by popularity
- ✅ **Popularity Score Display** - Fire emoji (🔥) with formatted score on posts
- ✅ **Trending Feed API** - Integrated `GET /api/v1/posts/trending` endpoint
- ✅ **Auto-recalculation** - Backend automatically updates scores on like/comment actions

**Files Modified:**
| File | Changes |
|------|---------|
| `community.service.ts` | Added `getTrendingPosts()` method |
| `patient-community.component.ts` | Added `activeFeed` state, `switchFeed()` method, `formatPopularity()` helper |
| `patient-community.component.html` | Added Latest/Trending tabs, popularity score badges |

---

### Feature: Anti-Toxicity Content Moderation
**Purpose:** Automatically detect and block inappropriate content (French + English) to maintain a safe community.

**Detection:** Word-list based (60+ French words, 60+ English words)

**Features Implemented:**
- ✅ **Real-time Content Validation** - Debounced (500ms) checking as user types
- ✅ **Toxic Content Warning** - Inline warning banner showing detected words
- ✅ **Submit Blocking** - Disable submit buttons when toxic content detected
- ✅ **Backend Error Handling** - Handle 400 Bad Request with `inappropriate-content` error type
- ✅ **Toast Notifications** - Show user-friendly error messages with detected words
- ✅ **Comment Moderation** - Same validation for comments in post detail

**New API Endpoints Integrated:**
- `POST /api/v1/moderation/check` - Check content toxicity
- `GET /api/v1/moderation/languages` - List supported languages
- `GET /api/v1/moderation/blocked-words/{lang}` - Get blocked words list

**Files Modified:**
| File | Changes |
|------|---------|
| `community.model.ts` | Added `ModerationResult`, `ToxicContentError`, `BlockedWordsResponse`, `ContentCheckRequest` interfaces |
| `community.service.ts` | Added `checkContent()`, `getModerationLanguages()`, `getBlockedWords()`, `checkContentSafe()` methods |
| `patient-community.component.ts` | Added content checking with Subject/debounce pattern, toxic content handling |
| `patient-community.component.html` | Added toxic content warning banner, content moderation info box |
| `post-detail.component.ts` | Added comment content validation, toxic content handling |
| `post-detail.component.html` | Added comment toxic content warning, moderation info box |
