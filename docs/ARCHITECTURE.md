...
| Community Social | 8009 | `/api/v1/community` | ✅ Complete + Popularity + Moderation |
...
## 🆕 Community Social Service Enhancements (2026-03-04)

### Overview
The Community Social Service has been enhanced with two advanced business features:
...
### 1. Popularity Score System

**Formula:** `score = (likeCount × 2) + (commentCount × 5) + max(0, 100 - hoursSincePost)`

**Features:**
- Auto-recalculates on every like/comment action
- New endpoint: `GET /api/v1/posts/trending` - Returns posts sorted by popularity
- Backend automatically updates `popularityScore` field on posts
- `commentCount` field tracks total comments per post

**Frontend Integration:**
| Component | Enhancement |
|-----------|-------------|
| `CommunityService` | Add `getTrendingPosts()` method |
| `PatientCommunityComponent` | Add "Trending" tab/filter |
| `PostDetailComponent` | Display popularity score |

### 2. Anti-Toxicity System

**Detection Method:** Word-list based filtering
- **French:** 60+ words (connard, salope, pute, merde, enculé, etc.)
- **English:** 60+ words (stupid, idiot, hate, kill, shit, fuck, etc.)

**Behavior:**
- Content rejected immediately with `400 Bad Request`
- Error response includes detected words list
- Moderation endpoints for pre-flight checks

**Error Response Format:**
```json
{
  "type": "https://alzcare.com/errors/inappropriate-content",
  "title": "Inappropriate Content Detected",
  "status": 400,
  "detail": "Content contains inappropriate language: [con, merde]",
  "detectedWords": ["con", "merde"],
...
|----------|--------|-------------|
| `/api/v1/moderation/check` | POST | Pre-check content toxicity |
| `/api/v1/moderation/languages` | GET | List supported languages |
| `/api/v1/moderation/blocked-words/{lang}` | GET | View blocked words list |

**Frontend Integration:**
| Component | Enhancement |
|-----------|-------------|
| `CommunityService` | Add moderation methods |
| `PatientCommunityComponent` | Real-time content validation before submit |
| `PostDetailComponent` | Comment toxicity check |
| Error Handler | Show detected words in toast notification |
...
*Last Updated: 2026-03-04 (Session: Community Enhancements - Popularity Score + Anti-Toxicity)*
